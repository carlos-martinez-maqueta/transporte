...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT * FROM tbl_destino WHERE (nombre = :nombre) AND id != :id";
    ...
    $result = Destination::editDestinationId($id, $nombre, $estado);
    ...
    if ($result->execute()) {
        ...
        $response = array(
            'status' => 'success',
            'message' => 'El destino se edito correctamente.'
        );
    } else {
        // Error al registrar Items
        $response = array(
            'status' => 'error',
            'message' => 'Error al editar destino.'
        );
    }
